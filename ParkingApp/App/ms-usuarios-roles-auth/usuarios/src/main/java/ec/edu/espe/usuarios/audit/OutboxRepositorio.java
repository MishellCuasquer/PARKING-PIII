...
public interface OutboxRepositorio extends JpaRepository<OutboxEvent, UUID> {
    ...
    /**
     * Eventos aÃºn sin publicar, del mÃ¡s antiguo al mÃ¡s reciente: la auditorÃ­a
     * debe conservar el orden en que ocurrieron los hechos, no el orden en que
     * se recuperÃ³ el broker.
     *
     * Se toman con bloqueo y SKIP LOCKED (lock.timeout = -2) porque el servicio
     * corre con varias rÃ©plicas en Kubernetes: sin esto, dos pods leerÃ­an las
     * mismas filas a la vez y publicarÃ­an el evento por duplicado. Con SKIP
